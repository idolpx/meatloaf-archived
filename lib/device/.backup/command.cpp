...
static log_t command_log = LOG_ERR;



#if 0
const char *command_errortext(unsigned int code)
{
...
{
    command_log = log_open("VDriveCommand");
}

/* called by vdrive-iec.c; return code not used */
int command_execute(vdrive_t *vdrive, const uint8_t *buf,
                           unsigned int length)
{
    ...
    if (length > IP_MAX_COMMAND_LEN) {
        command_set_error(vdrive, CBMDOS_IPE_LONG_LINE, 0, 0);
    ...
    switch (buf[0]) {
        case 'G': /* get partition info */
            status = command_getpartinfo(vdrive, buf, length);
            goto out3;
        case 'D': /* delete partition */
            status = command_deletepart(vdrive, buf, length);
            goto out3;
        case 'M': /* Memory */
            if (length > 2 && buf[1] == '-') {
                status = command_memory(vdrive, (uint8_t *)buf, length);
                goto out3;
            }
            break;
    }
    ...
    if (!vdrive->image) {
        return command_set_error(vdrive, CBMDOS_IPE_NOT_READY, 0, 0);
    }
#endif
    ...
    switch (*p) {
        case 'P': /* Position - update error channel */
            status = command_position(vdrive, p, length);
            command_set_error(vdrive, status, 0, 0);
            goto out2;
        case 'T': /* Time - don't update status - it does */
            status = command_time(vdrive, p, length);
            goto out2;
    ...
    switch (cmd.command[0]) {
        case 'C':       /* CD, CP, copy moves to next case */
            if (cmd.commandlength > 1 && cmd.command[1] == 'D'
                ...
                status = command_chdir(vdrive, &cmd);
                goto out;
            } else if (cmd.commandlength > 1 && (cmd.command[1] & 0x7f) == 'P') {
                status = command_chcmdpart(vdrive, &cmd);
                /* it sets the error */
                goto out1;
            }
            break;
        ...
        case 'U': /* User */
            if (cmd.commandlength > 1) {
                switch (cmd.command[1]) {
                    ...
                    case 'B': /* UB */
                    case '2': /* U2 */
                        status = command_u1a2b(vdrive, &cmd);
                        goto out1;
                    ...
                    case '9': /* U9 */
                        if (cmd.commandlength == 3 && (cmd.command[2] == '-' || cmd.command[2] == '+')) {
                            ...
                            if (!command_initialize(vdrive, NULL)) {
                    ...
                    case 'J': /* UJ */ /* CMDHD/FD2000/F4000 also have UJP to reload partition table */
                        if (cmd.commandlength > 2 && cmd.command[2] == 'P' &&
                            ...
                            if (!vdrive_read_partition_table(vdrive)) {
                                ...
                                if (command_switch(vdrive, vdrive->default_part)) {
        ...
        case 'M':       /* MD - make sub dir */
            if (cmd.commandlength > 1 && cmd.command[1] == 'D' &&
                vdrive->image_format == VDRIVE_IMAGE_FORMAT_NP) {
                status = command_mkdir(vdrive, &cmd);
                goto out;
            }
            break;
        ...
        case 'R':       /* RD */
            if (cmd.commandlength > 1 && cmd.command[1] == 'D' &&
                vdrive->image_format == VDRIVE_IMAGE_FORMAT_NP) {
                status = command_rmdir(vdrive, &cmd);
                goto out1;
            }
            break;
    }
    ...
    switch (cmd.abbrv[0]) {
        ...
        case 'R':       /* R-H, R-P, or R */
            if (cmd.abbrvlength > 2 && cmd.abbrv[1] == '-' && cmd.abbrv[2] == 'H') {
                status = command_renameheader(vdrive, &cmd);
            } else if (cmd.abbrvlength > 2 && cmd.abbrv[1] == '-' && cmd.abbrv[2] == 'P') {
                status = command_renamepart(vdrive, &cmd);
            } else {
                status = command_rename(vdrive, &cmd);
            }
            break;

        case 'L':       /* Lock/Unlock */
            status = command_lockunlock(vdrive, &cmd);
            break;

        case 'S':       /* Scratch */
            status = command_scratch(vdrive, &cmd);
            goto out1; /* error message is set by function */

        case 'I':       /* initialize */
            status = command_initialize(vdrive, &cmd);
            break;

        case 'N':       /* new */
            status = command_format_internal(vdrive, &cmd);
            break;

        case 'V':       /* validate */
            status = command_validate_internal(vdrive, &cmd);
            goto out1; /* error message is set by function */

        case 'B':       /* Block, Buffer */
            status = command_block(vdrive, &cmd);
            goto out1; /* error message is set by function */
...
out:
    ...
    if (status == CBMDOS_IPE_INVAL) {
        log_error(command_log, "Wrong command `%s'.", p);
    }

    command_set_error(vdrive, status, statret1, 0);
...
static int command_get_block_parameters(char *buf, int *p1, int *p2, int *p3,
                                               int *p4)
{
    int ip;
    char *bp, endsign;
...
static int command_u1a2b(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status;
...
#ifdef DEBUG_DRIVE
    log_debug("command_ua1b2 command: %c.", cmd->command[1]);
#endif
    ...
    if (cmd->commandlength > 1 && cmd->filelength > 5) {
        rc = command_get_block_parameters((char*)cmd->file, &channel,
                                                 &drive, &track, &sector);
        if (rc < 0) {
#ifdef DEBUG_DRIVE
            ...
            if (command_switch(vdrive, drive)) {
                track = 0;
                sector = 0;
                goto out;
            }
            ...
            log_error(command_log, "U1/A/2/B invalid parameter "
...
out:
    ...
    command_return(vdrive, origpart);

    vdrive->last_code = CBMDOS_IPE_OK;

    return command_set_error(vdrive, status, track, sector);
}


static int command_block(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status;
    /* set all parameters to 0 by default */
...
#ifdef DEBUG_DRIVE
    log_debug("command_block command: %c.", cmd->abbrv[2]);
#endif
    ...
    switch (cmd->abbrv[2]) {
        ...
        case 'R':
        case 'W':
            l = command_get_block_parameters((char*)cmd->file, &channel,
                                                    &drive, &track, &sector);
            ...
            if (l < 0) {
                ...
                log_error(command_log, "b-r/w invalid parameter "
                          "C:%i D:%i T:%i S:%i.", channel, drive, track, sector);
                status = l;
                goto out;
            }
        ...
        case 'F':
            ...
            l = command_get_block_parameters((char*)cmd->file, &drive,
        ...
        case 'P':
            ...
            break;
        case 'E':
            log_warning(command_log, "B-E: %d %d %d %d (needs TDE)", channel, drive, track, sector);
            status = CBMDOS_IPE_OK;
            goto out;
            break;
        default:
...
out:
    ...
    return command_set_error(vdrive, status, t, s);
}
...
*/
static int command_memory(vdrive_t *vdrive, uint8_t *buffer, unsigned int length)
{
    uint16_t addr;

    if (length < 5) {
        return command_set_error(vdrive, CBMDOS_IPE_SYNTAX, 0, 0);
    }
    ...
    switch (buffer[2]) {
        case 'W':
            return command_memory_write(vdrive, &(buffer[5]), addr, length);
        case 'R':
            return command_memory_read(vdrive, &(buffer[5]), addr, length);
        case 'E':
            return command_memory_exec(vdrive, NULL, addr, length);
    }

    return command_set_error(vdrive, CBMDOS_IPE_INVAL, 0, 0);
}
...
#define COPYMAX 6
static int command_copy(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status, rc;
    ...
    for (i = 0; i < files; i++) {
        ...
        status = command_switchtraverse(vdrive, &(src[i]));
    ...
    status = command_switchtraverse(vdrive, &dest);
    if (status) {
        goto out;
    }
    ...
    if (slot) {
        ...
        for (i = 0; i < files; i++) {
            ...
            do {
                /* read an input chunk */
                rn = 0;
                command_switch(vdrive, src[i].drive);
                ...
                command_switch(vdrive, dest.drive);
...
static int command_rename(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status, rc;
    uint8_t *slot;
    vdrive_dir_context_t dir;
...
/* backup current partition information */
    ...
    status = command_switchtraverse(vdrive, cmd);
    if (status) {
        goto out;
    }
...
static int command_renameheader(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status, rc;
    uint8_t tmp[256];
    int i;
...
static int command_renamepart(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status, rc;
    uint8_t *slot;
    vdrive_dir_context_t dir;
...
static int command_lockunlock(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status;
    uint8_t *slot;
    vdrive_dir_context_t dir;
...
int command_switchtraverse(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status, rc;
    uint8_t *slot, *next;
    vdrive_dir_context_t dir;
    ...
    if (command_switch(vdrive, cmd->drive)) {
        goto out;
    }
...
void command_return(vdrive_t *vdrive, int origpart)
{
...
int command_switch(vdrive_t *vdrive, int part)
{
...
static int command_scratch(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status, rc;
    int t, s, l;
...
out:
    ...
    command_set_error(vdrive, status, deleted_files, 0);
...
*/
static int command_chcmdpart(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status = CBMDOS_IPE_INVAL, statret1 = 0;
    int floppy, hd, np = 0;
...
out:
    return command_set_error(vdrive, status, statret1, 0);
}

/*
    CMD style subdir support (using DIR filetype)
*/
static int command_chdir(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status, p;
...
*/
static int command_chpart(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status;
...
out:
    ...
    command_set_error(vdrive, status, ts, te);

    return status;

}

static int command_mkdir(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status;
    uint8_t *slot;
    vdrive_dir_context_t dir;
...
static int command_rmdir(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status;
    uint8_t *slot;
    vdrive_dir_context_t dir;
...
/* backup current partition information */
    ...
    if (command_switchtraverse(vdrive, cmd)) {
...
out:
    ...
    return command_set_error(vdrive, status, deleted_files, 0);
}

static int command_initialize(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status;
/* backup current partition information */
    int origpart = vdrive->current_part;
...
static int command_allocate_chain(vdrive_t *vdrive, unsigned int t, unsigned int s, unsigned int *c)
{
    uint8_t tmp[256];
    int rc;
    ...
    while (t) {
        ...
        if (disk_image_check_sector(vdrive->image, t, s) < 0) {
            command_set_error(vdrive, CBMDOS_IPE_ILLEGAL_TRACK_OR_SECTOR,
                                     s, t);
        ...
        if (!vdrive_bam_allocate_sector(vdrive, t, s)) {
            ...
            command_set_error(vdrive, CBMDOS_IPE_NO_BLOCK, s, t);
...
static int command_validate_internal(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status;
...
/* backup current partition information */
    ...
    status = command_validate(vdrive);
...
static int command_validate_worker(vdrive_t *vdrive, int geos, unsigned int *t_passed, unsigned int *s_passed)
{
        ...
        /* If it is a closed/complete file ... */
            ...
            if ((*filetype & 0x07) == CBMDOS_FT_CBM) {
                ...
                if (b[SLOT_GEOS_TYPE] && geos && (*filetype & 0x07) != CBMDOS_FT_REL) {
                    ...
                    if (t) {
                        if (!vdrive_bam_allocate_sector(vdrive, t, s)) {
                            command_set_error(vdrive, CBMDOS_IPE_NO_BLOCK, t, s);
                            goto bad;
                        }
                    }
                    l++;
...
int command_validate(vdrive_t *vdrive)
{
    ...
    command_set_error(vdrive, CBMDOS_IPE_OK, 0, 0);
    ...
    if (vdrive->image_format != VDRIVE_IMAGE_FORMAT_9000) {
        status = command_allocate_chain(vdrive, vdrive->Bam_Track,
...
out:
    ...
    if (vdrive->last_code != CBMDOS_IPE_OK ) {
        command_set_error(vdrive, status, t, s);
    }

    return status;
}

static int command_format_internal(vdrive_t *vdrive, cbmdos_cmd_parse_plus_t *cmd)
{
    int status;
...
/* backup current partition information */
    ...
    if (VDRIVE_IS_FD(vdrive) && more) {
        ...
        for (i = 0; i < parts; i++) {
            ...
            if (command_switch(vdrive, i + 1)) {
                goto out;
            }
            if (longname) {
                longname[15] = '1' + i;
            }
            status = command_format_worker(vdrive, i == 0 ? name : longname, id);
            if (status) {
                goto out;
            }
        }
    ...
    status = command_format_worker(vdrive, name, id);
...
static int command_format_worker(struct vdrive_s *vdrive, uint8_t *disk_name, uint8_t *disk_id)
{
    uint8_t tmp[256];
    int status;
    uint8_t id[2];
...
static int command_position(vdrive_t *vdrive, uint8_t *buf,
                                   unsigned int length)
{
...
int command_set_error(vdrive_t *vdrive, int code, unsigned int track,
                              unsigned int sector)
{
    const char *message = "";
    bufferinfo_t *p = &vdrive->buffers[15];
...
/*    last_code = CBMDOS_IPE_OK; */
    ...
    if (code && code != CBMDOS_IPE_DOS_VERSION) {
        log_message(command_log, "ERR = %02d, %s, %02u, %02u",
...
/* FIXME: incomplete */
int command_memory_write(vdrive_t *vdrive, const uint8_t *buf, uint16_t addr, unsigned int length)
{
...
log_warning(LOG_DEFAULT,"job #%d write sector %u %u",i,(unsigned int)vdrive->ram[tracksector + (i << 1)], (unsigned int)vdrive->ram[tracksector + (i << 1) + 1]);
                ...
                case 0xd0:
                case 0xe0:
                    log_warning(command_log, "M-W %04x %u (+%u) (Job Queue Execute Function - needs TDE)", addr, len, length - 6);
                    break;

                default:
                    log_warning(command_log, "Unknown job code: %02x\r\n", (unsigned int)job);
                    break;
            }
        }
    }
...
good:
    ...
    return command_set_error(vdrive, CBMDOS_IPE_OK, 0, 0);
}

/* FIXME: This function doesn't need buf or length. */
int command_memory_exec(vdrive_t *vdrive, const uint8_t *buf, uint16_t addr, unsigned int length)
{
    if (length < 5) {
        log_warning(command_log,
    ...
    log_warning(command_log, "M-E %04x (+%u) (needs TDE)", addr, length - 5);
    return command_set_error(vdrive, CBMDOS_IPE_OK, 0, 0);
}
...
*
*/
int command_memory_read(vdrive_t *vdrive, const uint8_t *buf, uint16_t addr, unsigned int length)
{
    unsigned int len = buf[0];
    int i;
    bufferinfo_t *p = &vdrive->buffers[15];
    ...
    if (length < 6) {
        ...
        log_warning(command_log, "M-R %04x %u (+%u) (might need TDE)",
                addr, len, length - 6);
    }
...
static int command_time(vdrive_t *vdrive, uint8_t *cmd, int length)
{
...
bad:
    command_set_error(vdrive, CBMDOS_IPE_INVAL, 0, 0);
    return CBMDOS_IPE_INVAL;
}

static int command_getpartinfo(vdrive_t *vdrive, const uint8_t *cmd, int length)
{
    bufferinfo_t *p = &vdrive->buffers[15];
    /* backup current partition information */
    int origpart = vdrive->current_part;
...
static int command_deletepart(vdrive_t *vdrive, const uint8_t *cmd, int length)
{
    int maxpart;
    int i;
