...
int16_t  CPBStandardSerial::receiveByte(uint8_t device)
{
	flags = CLEAR;
...
// a printer chugging out a line of print, or a disk drive with a formatting job in progress,
// it might holdback for quite a while; there's no time limit.
bool CPBStandardSerial::sendByte(uint8_t data, bool signalEOI)
{
	flags = CLEAR;
...
int16_t CPBStandardSerial::timeoutWait(byte iecPIN, bool lineStatus, size_t wait, size_t step)
{
